const DEFAULT_CONTACT_TO_EMAIL = "jeremie.nagi@epitech.eu";
...
export const handleContactRequest = async (request, env) => {
  ...
  const response = await fetch("https://api.resend.com/emails", {
    ...
    body: JSON.stringify({
      from: fromEmail,
      to: env.CONTACT_TO_EMAIL || DEFAULT_CONTACT_TO_EMAIL,
      reply_to: email,
  ...
  if (!response.ok) {
    let resendError = "Le service d'envoi email a refusé la demande.";

    try {
      const data = await response.json();
      resendError = data?.message || data?.error || resendError;
    } catch {
      resendError = (await response.text()) || resendError;
    }
